...
export class WSDL {
  ...
  public xmlKey = "$xml";
  public attributesKey = "$attributes";
    ...
    p.onclosetag = (nsName) => {
      ...
      if (
        ...
        if (typeof obj === "object" && Object.keys(obj).length === 0) {
          obj = cur.object = this.options.preserveWhitespace
            ? cur.text || ""
            : "";
        }
      }
      ...
      if (root.Envelope) {
        ...
        if (body && body.Fault) {
          ...
          code =
            ...
            (fault.Code &&
              `${fault.Code.Value}: ${
                fault.Code.Subcode && fault.Code.Subcode.Value
              }`);
            ...
            if (schema) {
              ...
              if (
                ...
                if (childSchemaObject.$type) {
                  ...
                  const typePrefix =
                    childSchemaObject.$baseNameSpace || typeQName.prefix;
                  const typeURI =
  ...
  private _initializeOptions(options: any) {
    ...
    this.options.attributesKey = options.attributesKey || this.attributesKey;
  ...
  private _processNextInclude(includes: elements.IInclude[], callback) {
    ...
    if (
      ...
      try {
        includePath = this.options.wsdl_options.overrideImportLocation(
          includePath,
          this.uri,
          include.location,
          options
